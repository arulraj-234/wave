...
// HeroBanner component removed per user request for UI refinement
  ...
  return (
    <div className={`hidden md:block relative w-full max-w-[1400px] mx-auto transition-all duration-[1200ms] ease-[cubic-bezier(0.25,1,0.5,1)] ${currentSong ? 'h-[320px] opacity-100 mt-6 mb-10' : 'h-0 opacity-0 my-0 overflow-hidden'}`}>
      <div className="absolute inset-0 mx-8 rounded-3xl overflow-hidden bg-white/[0.02] border border-white/[0.05] shadow-[0_8px_32px_rgba(0,0,0,0.5)] flex items-center p-10 group">
        ...
        {/* Content Container */}
        <div className="relative z-10 flex items-center justify-between gap-14 w-full h-full px-12">
          
          {/* Typography (Left Side) */}
          <div className="flex-1 min-w-0 pr-8 transform transition-all duration-700 -translate-x-8 opacity-0 group-hover:translate-x-0 group-hover:opacity-100" style={{ opacity: 1, transform: 'translateX(0px)' }}>
             <h2 className="text-5xl md:text-6xl font-black tracking-tighter text-white mb-3 truncate drop-shadow-2xl">
               {currentSong.title}
             </h2>
             <p className="text-xl text-white/60 font-medium truncate mb-6">
               {currentSong.artist_name}
             </p>
          </div>

          {/* 3D Vinyl Player Assembly (Right Side) */}
          <div className="relative w-56 h-56 shrink-0 flex items-center justify-end">
            
            {/* The Record Wrapper (Handles Slide Out) */}
            <div className={`absolute right-0 w-56 h-56 transition-all duration-[1200ms] ease-[cubic-bezier(0.2,0.8,0.2,1)] ${isPlaying ? '-translate-x-32 opacity-100' : '-translate-x-4 opacity-20'}`}>
               ...
               >
                 ...
                 {/* Center Label */}
                 <div className="w-[84px] h-[84px] rounded-full border-[3px] border-black overflow-hidden relative shadow-[inset_0_0_10px_rgba(0,0,0,0.8)]">
            ...
            <div className="absolute right-0 w-56 h-56 rounded-xl overflow-hidden shadow-[-20px_0_50px_rgba(0,0,0,0.5)] z-10 border border-white/10 bg-brand-surface transform transition-transform duration-700 group-hover:scale-105 group-hover:-translate-y-2">
