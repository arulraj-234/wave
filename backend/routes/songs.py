...
def enrich_song_metadata(songs):
    ...
    # Optimization: Skip items that already have 'artists' metadata to minimize IN clause size
    ...
    # Attach to each song
    for s in songs:
        # Optimization: Skip overwriting existing metadata with empty defaults
        if 'artists' in s:
            continue

        artists = artists_by_song.get(s.get('song_id'), []) if artists_by_song else []
