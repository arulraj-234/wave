...
def get_artist_stats(artist_id):
    ...
    # ⚡ Bolt Optimization: Cache the complex aggregation query results for 1 hour.
    # Impact: Reduces database load significantly by avoiding ~16 heavy analytical queries per request.
    # We cache the raw payload dict instead of the Flask Response for thread safety.
